...
from api.models.crud import create_request, update_request
...
async def full_provision_test(id: int, request_body: RequestsCreate, session: Session = Depends(get_session)):
    ...
    try:
        ...
        # 네임스페이스 생성
        process.create_namespace(namespace=namespace)

        # AWS 자격 증명 생성
        process.create_aws_credentials(
        ...
        update_request(session=session, request_data=data)
        raise HTTPException(status_code=500, detail="처리 중 오류 발생")

    # 프로비저닝 진행 중 상태 업데이트
    update_request(
        session=session, request_data={"progress": "프로비저닝", "processState": "성공", "provisionState": "시작"}
    )

    try:
        # Pod 생성
        pod_name = "fullProvisionTest-"+str(id)
        provision.create_pod(name=pod_name)

    except Exception as e:
        # 에러가 발생하면 요청 상태를 실패로 업데이트하고 HTTP 예외를 발생시킵니다.
        update_request(session=session, request_data={"progress": "프로비저닝", "provisionState": "실패"})
        print(e)
        raise HTTPException(status_code=500, detail="Pod 생성 중 오류 발생")

    # 프로비저닝 완료 상태 업데이트
    update_request(
        session=session, request_data={"progress": "프로비저닝", "provisionState": "성공"}
    )

    return {"message": f"{id}에 대한 전체 프로비저닝 테스트 완료"}

@router.post("/deploytest/{id}", tags=["[1013]test"])
def deploy_test(id: int):
    return {"message": f"Full provision test for {id}"}
